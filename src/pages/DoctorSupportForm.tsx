...
const DoctorSupportForm = () => {
  ...
  const handlePrint = () => {
    setShowPreview(true);
    setTimeout(() => {
      printElement("doctor-support-print");
      setShowPreview(false);
    }, 800);
  };

  const PrintContent = () => (
    <div id="doctor-support-print" ref={printRef} className="print-page" style={{ border: "2px solid #000", borderRadius: "5px" }}>
      <FormHeader />
      ...
      <div className="top-half" style={{ marginBottom: "10px" }}>
        ...
        <div className="flex-row">
          ...
          <span className="dotted-line out-text">{formData.supervisor}</span>
        ...
        <div className="flex-row">
          ...
          <span className="dotted-line out-text">{formData.amount}</span>
          <span style={{ whiteSpace: "nowrap" }}>) فقط.</span>
        </div>
        <div className="flex-row">
          <div style={{ flexBasis: "55%", display: "flex", alignItems: "baseline" }}>
            <span style={{ whiteSpace: "nowrap" }}>للأخ الدكتور:</span><span className="dotted-line out-text">{formData.doctor}</span>
          </div>
          <div style={{ flexBasis: "42%", display: "flex", alignItems: "baseline" }}>
            <span style={{ whiteSpace: "nowrap" }}>أخصائي:</span><span className="dotted-line out-text">{formData.specialty}</span>
          </div>
        </div>
        <div className="flex-row">
          <div style={{ flexBasis: "48%", display: "flex", alignItems: "baseline" }}>
            <span style={{ whiteSpace: "nowrap" }}>يعمل صباحاً في:</span><span className="dotted-line out-text">{formData.morning}</span>
          </div>
          <div style={{ flexBasis: "48%", display: "flex", alignItems: "baseline" }}>
            <span style={{ whiteSpace: "nowrap" }}>ومساءً في:</span><span className="dotted-line out-text">{formData.evening}</span>
          </div>
        </div>
        <div className="flex-row">
          <div style={{ flexBasis: "48%", display: "flex", alignItems: "baseline" }}>
            <span style={{ whiteSpace: "nowrap" }}>تلفون ثابت:</span><span className="dotted-line out-text">{formData.landline}</span>
          </div>
          <div style={{ flexBasis: "48%", display: "flex", alignItems: "baseline" }}>
            <span style={{ whiteSpace: "nowrap" }}>تلفون سيار:</span><span className="dotted-line out-text">{formData.mobile}</span>
          </div>
        </div>
        <div className="flex-row">
          <span style={{ whiteSpace: "nowrap" }}>مقابل / </span><span className="dotted-line out-text">{formData.purpose}</span>
        </div>
        <div className="flex-row">
          <span style={{ whiteSpace: "nowrap" }}>لكتابة الأصناف التالية: </span><span className="dotted-line out-text">{formData.items}</span>
        </div>
          ...
          <table className="compact-table">
            ...
            <tbody>
              {pharmacies.length === 0 ? (
                ...
                <tr key={i}><td>{p.name}</td><td dir="ltr">{p.phone}</td><td>{p.amount}</td></tr>
              ))}
            </tbody>
          </table>
        </div>
    ...
    <div className="container mx-auto px-4 py-6 animate-fade-in">
      ...
      <Dialog open={showPreview} onOpenChange={setShowPreview}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>معاينة استمارة دعم طبيب</DialogTitle></DialogHeader>
          <PrintContent />
        </DialogContent>
      </Dialog>
    </div>
